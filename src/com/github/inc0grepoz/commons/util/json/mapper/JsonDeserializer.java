...
public class JsonDeserializer
{
    ...
    throws Throwable
    {
        ...
        // Resolve type parameters: use provided ones or get from class definition
        if (typeParameters.length == 0) {
            typeParameters = getClassTypeParameters(clazz);
        }
        ...
        if (Collection.class.isAssignableFrom(clazz))
        {
            ...
            // Creating an implemented instance of the given class
    ...
    throws Throwable
    {
        if (map)
        {
            // Default to String.class if no type parameters are provided
            Class<?> keyType = typeParameters.length > 0 ? typeParameters[0] : String.class;
            Class<?> valueType = typeParameters.length > 1 ? typeParameters[1] : String.class;

            // Deserializing and putting the object into the map
            ((Map) instance).put(
                deserialize(fieldName, keyType),
                deserialize(json, valueType)
            );
        }
        else
        {
    ...
    // Extracts generic type parameters from a class's superclass definition.
    private Class<?>[] getClassTypeParameters(Class<?> clazz)
    {
        ...
        if (superClass instanceof ParameterizedType)
        {
            ...
            Class[] atac = new Class[ata.length];

            for (int i = 0; i < ata.length; i++)
            {
                if (ata[i] instanceof Class)
                {
                    atac[i] = (Class<?>) ata[i];
                }
                else if (ata[i] instanceof ParameterizedType)
                {
                    atac[i] = (Class<?>) ((ParameterizedType) ata[i]).getRawType();
                }
                else
                {
                    atac[i] = Object.class; // fallback
                }
            }
            return atac;
        }

        return new Class<?>[0]; // no generic parameters
    }
